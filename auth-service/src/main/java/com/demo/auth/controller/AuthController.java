...
public class AuthController {
    ...
    @PostMapping("/token")
    public ResponseEntity<TokenResponse> token(@RequestBody @Valid LoginRequest loginRequest) throws Exception {
    ...
    @Operation(summary = "Logout global", description = "Revokes all refresh tokens for the current user")
    @ApiResponse(responseCode = "204", description = "All tokens revoked successfully")
    @PostMapping("/logout-global")
    public ResponseEntity<Void> logoutGlobal(Authentication authentication) {
